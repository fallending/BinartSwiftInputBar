...
extern const NSTimeInterval kBAAnimatedImageDelayTimeIntervalMinimum;

//
//  An `BAAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `BAAnimatedImageView`.
...
//
@interface BAAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `BAAnimatedImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `BAAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
...
@interface BAAnimatedImage (Logging)
...
#define FLLog(logLevel, format, ...) [BAAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]
