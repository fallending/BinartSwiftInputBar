...
@protocol BAAnimatedImageViewDebugDelegate <NSObject>
@optional
- (void)debug_animatedImageView:(BAAnimatedImageView *)animatedImageView waitingForFrame:(NSUInteger)index duration:(NSTimeInterval)duration;
@end
#endif


@interface BAAnimatedImageView ()
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<BAAnimatedImageViewDebugDelegate> debug_delegate;
#endif

@end


@implementation BAAnimatedImageView
...
- (void)setAnimatedImage:(BAAnimatedImage *)animatedImage {
    if (![_animatedImage isEqual:animatedImage]) {
        if (animatedImage) {
            if (super.image) {
                ...
                // See more here: https://github.com/Flipboard/BAAnimatedImage/issues/100
...
{
    // Presision is set to half of the `kBAAnimatedImageDelayTimeIntervalMinimum` in order to minimize frame dropping.
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kBAAnimatedImageDelayTimeIntervalMinimum;
