...
const NSTimeInterval kBAAnimatedImageDelayTimeIntervalMinimum = 0.02;

// An animated image's data size (dimensions * frameCount) category; its value is the max allowed memory (in MB).
// E.g.: A 100x200px GIF with 30 frames is ~2.3MB in our pixel format and would fall into the `BAAnimatedImageDataSizeCategoryAll` category.
typedef NS_ENUM(NSUInteger, BAAnimatedImageDataSizeCategory) {
    BAAnimatedImageDataSizeCategoryAll = 10,       // All frames permanently in memory (be nice to the CPU)
    BAAnimatedImageDataSizeCategoryDefault = 75,   // A frame cache of default size in memory (usually real-time performance and keeping low memory profile)
    BAAnimatedImageDataSizeCategoryOnDemand = 250, // Only keep one frame at the time in memory (easier on memory, slowest performance)
    BAAnimatedImageDataSizeCategoryUnsupported     // Even for one frame too large, computer says no.
};

typedef NS_ENUM(NSUInteger, BAAnimatedImageFrameCacheSize) {
    BAAnimatedImageFrameCacheSizeNoLimit = 0,                // 0 means no specific limit
    BAAnimatedImageFrameCacheSizeLowMemory = 1,              // The minimum frame cache size; this will produce frames on-demand.
    BAAnimatedImageFrameCacheSizeGrowAfterMemoryWarning = 2, // If we can produce the frames faster than we consume, one frame ahead will already result in a stutter-free playback.
    BAAnimatedImageFrameCacheSizeDefault = 5                 // Build up a comfy buffer window to cope with CPU hiccups etc.
};


#if defined(DEBUG) && DEBUG
@protocol BAAnimatedImageDebugDelegate <NSObject>
@optional
- (void)debug_animatedImage:(BAAnimatedImage *)animatedImage didUpdateCachedFrames:(NSIndexSet *)indexesOfFramesInCache;
- (void)debug_animatedImage:(BAAnimatedImage *)animatedImage didRequestCachedFrame:(NSUInteger)index;
- (CGFloat)debug_animatedImagePredrawingSlowdownFactor:(BAAnimatedImage *)animatedImage;
@end
#endif


@interface BAAnimatedImage ()
...
@property (nonatomic, strong, readonly) BAAnimatedImage *weakProxy;

#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<BAAnimatedImageDebugDelegate> debug_delegate;
#endif
...
@implementation BAAnimatedImage
...
- (NSUInteger)frameCacheSizeCurrent {
    ...
    if (self.frameCacheSizeMax > BAAnimatedImageFrameCacheSizeNoLimit) {
        frameCacheSizeCurrent = MIN(frameCacheSizeCurrent, self.frameCacheSizeMax);
    }
    
    if (self.frameCacheSizeMaxInternal > BAAnimatedImageFrameCacheSizeNoLimit) {
...
+ (void)initialize
{
    if (self == [BAAnimatedImage class]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationDidReceiveMemoryWarningNotification object:nil queue:nil usingBlock:^(NSNotification *note) {
            ...
            // Note that BAAnimatedImage can be created on any thread, so the hash table must be locked.
            NSArray *images = nil;
...
- (instancetype)init {
    BAAnimatedImage *animatedImage = [self initWithAnimatedGIFData:nil];
    if (!animatedImage) {
        FLLog(FLLogLevelError, @"Use `-initWithAnimatedGIFData:` and supply the animated GIF data as an argument to initialize an object of type `BAAnimatedImage`.");
    }
...
- (instancetype)initWithAnimatedGIFData:(NSData *)data optimalFrameCacheSize:(NSUInteger)optimalFrameCacheSize predrawingEnabled:(BOOL)isPredrawingEnabled {
    ...
    if (self) {
            ...
            @autoreleasepool {
                ...
                if (frameImageRef) {
                    ...
                    if (frameImage) {
                        ...
                        // Support frame delays as low as `kBAAnimatedImageDelayTimeIntervalMinimum`, with anything below being rounded up to `kDelayTimeIntervalDefault` for legacy compatibility.
                        ...
                        if ([delayTime floatValue] < ((float)kBAAnimatedImageDelayTimeIntervalMinimum - FLT_EPSILON)) {
                            FLLog(FLLogLevelInfo, @"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).", i, [delayTime floatValue], kDelayTimeIntervalDefault, kBAAnimatedImageDelayTimeIntervalMinimum);
                            delayTime = @(kDelayTimeIntervalDefault);
                        }
        ...
        if (optimalFrameCacheSize == 0) {
            ...
            if (animatedImageDataSize <= BAAnimatedImageDataSizeCategoryAll) {
                _frameCacheSizeOptimal = self.frameCount;
            } else if (animatedImageDataSize <= BAAnimatedImageDataSizeCategoryDefault) {
                ...
                _frameCacheSizeOptimal = BAAnimatedImageFrameCacheSizeDefault;
            } else {
                // The predicted size exceeds the limits to build up a cache and we go into low memory mode from the beginning.
                _frameCacheSizeOptimal = BAAnimatedImageFrameCacheSizeLowMemory;
            }
        } else {
        ...
        // Note that BAAnimatedImages can be created on any thread, so the hash table must be locked.
        @synchronized(allAnimatedImagesWeak) {
...
{
    BAAnimatedImage *animatedImage = [[BAAnimatedImage alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
- (void)addFrameIndexesToCache:(NSIndexSet *)frameIndexesToAddToCache
{
    ...
    BAAnimatedImage * __weak weakSelf = self;
...
+ (CGSize)sizeForImage:(id)image {
    ...
    } else if ([image isKindOfClass:[BAAnimatedImage class]]) {
        BAAnimatedImage *animatedImage = (BAAnimatedImage *)image;
        imageSize = animatedImage.size;
    } else {
        // Bear trap to capture bad images; we have seen crashers cropping up on iOS 7.
        FLLog(FLLogLevelError, @"`image` isn't of expected types `UIImage` or `BAAnimatedImage`: %@", image);
    }
...
{
    self.frameCacheSizeMaxInternal = BAAnimatedImageFrameCacheSizeNoLimit;
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification
{
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self.weakProxy selector:@selector(growFrameCacheSizeAfterMemoryWarning:) object:@(BAAnimatedImageFrameCacheSizeGrowAfterMemoryWarning)];
    ...
    FLLog(FLLogLevelDebug, @"Attempt setting frame cache size max to %lu (previous was %lu) after memory warning #%lu for animated image: %@", (unsigned long)BAAnimatedImageFrameCacheSizeLowMemory, (unsigned long)self.frameCacheSizeMaxInternal, (unsigned long)self.memoryWarningCount, self);
    self.frameCacheSizeMaxInternal = BAAnimatedImageFrameCacheSizeLowMemory;
    ...
    if ((self.memoryWarningCount - 1) <= kGrowAttemptsMax) {
        [self.weakProxy performSelector:@selector(growFrameCacheSizeAfterMemoryWarning:) withObject:@(BAAnimatedImageFrameCacheSizeGrowAfterMemoryWarning) afterDelay:kGrowDelay];
    }
...
@implementation BAAnimatedImage (Logging)
