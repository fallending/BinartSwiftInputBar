...
#import "UILayoutGuide+BALayout.h"

static inline UILayoutGuide * layout_guide(NSObject * view) {
    if (view && [view isKindOfClass:UILayoutGuide.self]) {
        return (UILayoutGuide *)view;
    }
    return nil;
}

static inline UIView * view(NSObject * view) {
    if (view && [view isKindOfClass:UIView.self]) {
        return (UIView *)view;
    }
    return nil;
}

static inline UIView * owningView(NSObject * view) {
    if (view) {
        if ([view isKindOfClass:UIView.self]) {
            return (UIView *)view;
        }
        if ([view isKindOfClass:UILayoutGuide.self]) {
            return ((UILayoutGuide *)view).owningView;
        }
    }
    return nil;
}
...
@implementation UILayoutGuide (WHC_AutoLayout)
...
- (LGRemoveConstraintToViewAttribute)whc_RemoveToLayoutAttrs {
    __weak typeof(self) weakSelf = self;
    return ^(NSObject * toView, NSLayoutAttribute attributes, ...) {
        va_list attrs;
        va_start(attrs, attributes);
        NSLayoutAttribute maxAttr = [weakSelf whc_GetMaxLayoutAttribute];
        while(attributes > NSLayoutAttributeNotAnAttribute && attributes <= maxAttr) {
...
- (LGLeftSpaceToView)whc_LeftSpaceToView {
    __weak typeof(self) weakSelf = self;
    return ^(CGFloat space, NSObject * toView) {
...
- (LGLeftSpaceEqualViewOffset)whc_LeftSpaceEqualViewOffset {
    __weak typeof(self) weakSelf = self;
    return ^(NSObject * view, CGFloat offset) {
...
- (LGLeadingSpaceToView)whc_LeadingSpaceToView {
    __weak typeof(self) weakSelf = self;
    return ^(CGFloat value , NSObject * toView) {
...
- (LGRightSpaceEqualViewOffset)whc_RightSpaceEqualViewOffset {
    __weak typeof(self) weakSelf = self;
    return ^(NSObject * toView, CGFloat offset) {
...
- (LGWidthEqualViewRatio)whc_WidthEqualViewRatio {
    __weak typeof(self) weakSelf = self;
    return ^(NSObject * view , CGFloat value) {
...
- (LGCenterToView)whc_CenterToView {
    __weak typeof(self) weakSelf = self;
    return ^(CGPoint center,NSObject * toView) {
...
- (void)whc_CommonRemoveConstraint:(NSLayoutAttribute)attribute view:(UIView *)mainView to:(NSObject *)toView {
...
- (void)whc_SwitchRemoveAttr:(NSLayoutAttribute)attr view:(UIView *)view to:(NSObject *)toView removeSelf:(BOOL)removeSelf {
    switch (attr) {
#if (__IPHONE_OS_VERSION_MIN_REQUIRED >= 80000) || (__TV_OS_VERSION_MIN_REQUIRED >= 9000) || (__MAC_OS_X_VERSION_MIN_REQUIRED >= 101100)
        case NSLayoutAttributeFirstBaseline:
#endif
...
- (UILayoutGuide *)whc_ClearLayoutAttrs {
    @autoreleasepool {
        ...
        UIView * superView = self.owningView;
        if (superView) {
...
- (UILayoutGuide *)whc_RemoveFrom:(UIView *)view layoutAttrs:(NSLayoutAttribute)attributes, ... {
    va_list attrs;
    va_start(attrs, attributes);
...
- (UILayoutGuide *)whc_HandleConstraintsRelation:(NSLayoutRelation)relation {
    ...
    if (constraints && constraints.relation != relation) {
        ...
        UIView * mainView = [self whc_MainViewConstraint:constraints];
        if (mainView) {
...
- (UILayoutGuide *)whc_HandleConstraintsPriority:(UILayoutPriority)priority {
    NSLayoutConstraint * constraints = [self currentConstraint];
...
#if (__IPHONE_OS_VERSION_MIN_REQUIRED >= 80000) || (__TV_OS_VERSION_MIN_REQUIRED >= 9000) || (__MAC_OS_X_VERSION_MIN_REQUIRED >= 101100)
- (NSObject *)whc_FirstBaseLine:(CGFloat)space {
    return [self whc_ConstraintWithItem:self.owningView
...
- (UILayoutGuide *)whc_ConstraintWithItem:(NSObject *)item
...
- (UILayoutGuide *)whc_ConstraintWithItem:(NSObject *)item
    ...
    UIView * superView = [self mainSuperView:toItem view2:item];
...
- (void)removeCacheConstraint:(NSLayoutConstraint *)constraint {
    UIView * mainView = [self whc_MainViewConstraint:constraint];
    if (mainView) {
...
- (UIView *)mainSuperView:(NSObject *)view1 view2:(NSObject *)view2 {
    BOOL isView1 = !(view1 && [view1 isKindOfClass:UILayoutGuide.self]);
    BOOL isView2 = !(view2 && [view2 isKindOfClass:UILayoutGuide.self]);
    if (isView1 && isView2) {
        UIView * s_view1 = view(view1);
        UIView * s_view2 = view(view2);
        if (!s_view1 && s_view2) {
            return s_view2;
        }
        if (s_view1 && !s_view2) {
        ...
        if (isView1 && !isView2) {
            if (view1) {
                UIView * s_view = view(view1);
                UILayoutGuide * guide1 = layout_guide(view2);
            ...
            if (view2) {
                UIView * s_view = view(view2);
                UILayoutGuide * guide1 = layout_guide(view1);
                if (s_view == guide1.owningView) {
                    return s_view;
                }else {
                    if (s_view.superview == guide1.owningView) {
...
- (UIView *)checkSubSuperView:(UIView *)superv subv:(UIView *)subv {
    UIView * superView;
    if (superv && subv && subv != superv) {
        UIView * sbvspv = subv.superview;
        if (sbvspv) {
            UIView * (^__block scanSubv)(NSArray<UIView *> *) = ^UIView * (NSArray<UIView *> * subvs) {
                ...
                if (subvs && subvs.count > 0) {
                    ...
                    if (!superView) {
                        NSMutableArray<UIView *> * sumSubv = [NSMutableArray array];
                        [subvs enumerateObjectsUsingBlock:^(UIView * _Nonnull sv, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (WHC_LGData *)sameSuperviewWithView1:(NSObject *)view1 view2:(NSObject *)view2 {
    WHC_LGData * data = [WHC_LGData new];
    data.isSameSuper = YES;
    BOOL isView1 = !(view1 && [view1 isKindOfClass:UILayoutGuide.self]);
    BOOL isView2 = !(view2 && [view2 isKindOfClass:UILayoutGuide.self]);
    if (isView1 && isView2) {
        UIView * tempToItem = view(view1);
        UIView * tempItem = view(view2);
        ...
        BOOL (^checkSameSuperview)(UIView *, UIView *) = ^(UIView * tmpSuperview, UIView * singleView) {
            UIView * tmpSingleView = singleView;
            if (tmpSingleView) {
                UIView * tempSingleSuperview = tmpSingleView.superview;
        ...
        if (tempToItem && tempItem) {
            UIView * toItemSuperview = tempToItem.superview;
            UIView * itemSuperview = tempItem.superview;
